  ...
  const rows = (Array.isArray(records) ? records : [])
    ...
    .map((record: any) => {
      ...
      if (Math.abs(moneyDelta(scheduledTotal, proposalTotal)) >= 0.01) {
        issues.push({
          code: 'schedule_total',
          label: 'CRM payment schedule',
          expected: proposalTotal,
          actual: scheduledTotal,
          delta: moneyDelta(scheduledTotal, proposalTotal),
        });
      }
      ...
      if (qbo.estimateId) {
        ...
        if (Math.abs(moneyDelta(estimateTotal, proposalTotal)) >= 0.01) {
          issues.push({
            code: 'estimate_total',
            label: 'QuickBooks estimate total',
            expected: proposalTotal,
            actual: estimateTotal,
            delta: moneyDelta(estimateTotal, proposalTotal),
          });
        }
      ...
      activeInvoices.forEach((invoice: any) => {
        ...
        if (!milestone) {
          issues.push({
            code: 'invoice_orphan',
            label: 'QuickBooks invoice ' + (invoice.docNumber || invoice.invoiceId),
            expected: null,
            actual: invoiceTotal,
            delta: null,
          });
        ...
        if (Math.abs(moneyDelta(invoiceTotal, milestoneAmount)) >= 0.01) {
          issues.push({
            code: 'invoice_milestone',
            label: (milestone.label || 'Milestone') + ' invoice',
            expected: milestoneAmount,
            actual: invoiceTotal,
            delta: moneyDelta(invoiceTotal, milestoneAmount),
          });
        }
      });
      ...
      if (Math.abs(moneyDelta(allocatedTotal, proposalTotal)) >= 0.01) {
        issues.push({
          ...
          label: 'Issued invoices + uninvoiced milestones',
          expected: proposalTotal,
          actual: allocatedTotal,
          delta: moneyDelta(allocatedTotal, proposalTotal),
        });
      }
      ...
      if (Math.abs(moneyDelta(openInvoiceBalance, expectedOpenInvoiceBalance)) >= 0.01) {
        issues.push({
          ...
          label: 'QuickBooks open invoice balance',
          expected: expectedOpenInvoiceBalance,
          actual: openInvoiceBalance,
          delta: moneyDelta(openInvoiceBalance, expectedOpenInvoiceBalance),
        });
      }
      if (qbo.balanceDue != null && Math.abs(moneyDelta(qbo.balanceDue, openInvoiceBalance)) >= 0.01) {
        issues.push({
          code: 'stored_balance',
          label: 'Stored QuickBooks balance',
          expected: openInvoiceBalance,
          actual: Math.round(Number(qbo.balanceDue || 0) * 100) / 100,
          delta: moneyDelta(qbo.balanceDue, openInvoiceBalance),
        });
      }
    ...
    .filter((row: any) =>
      row.estimateId || row.invoiceCount > 0 || ['accepted','booked'].includes(row.proposalStatus),
    )
    .sort((a: any, b: any) => {
...
export const config: Config = {
 ...
 + Number(proposal.subtotal || 0).toFixed(2),
    'Discount: 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + Number(proposal.discountAmount || 0).toFixed(2),
    'Hawaiʻi GET 4.712%: 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + Number(proposal.taxAmount || 0).toFixed(2),
    'Proposal total: 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + proposalTotal.toFixed(2),
    'Deposit (' + (Math.round(depositPercent * 1000) / 1000) + '%): 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + Number(proposal.depositAmount || 0).toFixed(2),
    'Payments received: 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + paymentsReceived.toFixed(2),
    'Remaining balance before this invoice: 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + remainingBalance.toFixed(2),
    'Remaining balance after this milestone is paid: 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
...
export const config: Config = {
 ...
 + remainingAfterMilestone.toFixed(2),
  ].join('\n');
  const created: any = await qboCreate(context, 'invoice', {
    ...
    PrivateNote: clean('Koa CRM ' + record.id + ' · ' + payment.label + ' · proposal total 
  const invoice = created?.Invoice;
  if (!invoice?.Id) throw new Error('QuickBooks invoice could not be created.');
