...
export type QuickBooksCustomPaymentPreset = {
  id: string;
  name: string;
  category: 'venueWedding' | 'mobileBar' | 'privateEvent' | 'default';
  depositPercent: number;
  milestones: QuickBooksPaymentTemplateItem[];
  createdAt: string;
  updatedAt: string;
};

export type QuickBooksPaymentRule = {
  id: string;
  name: string;
  category: 'venueWedding' | 'mobileBar' | 'privateEvent' | 'default';
...
export type QuickBooksDepositSettings = {
  ...
  customPresets: QuickBooksCustomPaymentPreset[];
  autoRules: QuickBooksPaymentRule[];
  updatedAt: string;
};
...
function cleanCustomPresets(input: unknown): QuickBooksCustomPaymentPreset[] {
  if (!Array.isArray(input)) return [];
  const now = new Date().toISOString();
  return input.slice(0, 50).map((item: any, index) => {
    const category = ['venueWedding','mobileBar','privateEvent','default'].includes(String(item?.category || ''))
      ? String(item.category) as QuickBooksCustomPaymentPreset['category']
      : 'default';
    return {
      id: String(item?.id || 'preset-' + (index + 1)).trim().slice(0, 80),
      name: String(item?.name || 'Custom preset').trim().slice(0, 120),
      category,
      depositPercent: cleanDepositPercent(item?.depositPercent, 10),
      milestones: cleanPaymentTemplate(item?.milestones, [{ label:'Final balance', dueDaysBefore:30, percentOfRemaining:100 }]),
...
function cleanAutoRules(input: unknown): QuickBooksPaymentRule[] {
  if (!Array.isArray(input)) return [];
  return input.slice(0, 100).map((item: any, index) => {
    const category = ['venueWedding','mobileBar','privateEvent','default'].includes(String(item?.category || ''))
      ? String(item.category) as QuickBooksPaymentRule['category']
      : 'default';
    ...
    const contractValueBasis = item?.contractValueBasis === 'beforeGet' ? 'beforeGet' : 'afterGet';
    return {
...
export async function getQuickBooksDepositSettings(context: Context): Promise<QuickBooksDepositSettings> {
  ...
  const defaultRules: QuickBooksPaymentRule[] = [
    { id:'rule-wedding-extended', name:'Wedding · 12+ months', category:'venueWedding', minLeadDays:365, maxLeadDays:null, minContractValue:null, maxContractValue:null, contractValueBasis:'afterGet', presetId:'builtin-extended-wedding', priority:10, active:true },
    { id:'rule-wedding-standard', name:'Wedding · 6–12 months', category:'venueWedding', minLeadDays:180, maxLeadDays:364, minContractValue:null, maxContractValue:null, contractValueBasis:'afterGet', presetId:'builtin-standard-wedding', priority:20, active:true },
    { id:'rule-wedding-micro', name:'Wedding · under 6 months', category:'venueWedding', minLeadDays:0, maxLeadDays:179, minContractValue:null, maxContractValue:null, contractValueBasis:'afterGet', presetId:'builtin-micro-wedding', priority:30, active:true },
    { id:'rule-mobile-bar', name:'Mobile Bar', category:'mobileBar', minLeadDays:null, maxLeadDays:null, minContractValue:null, maxContractValue:null, contractValueBasis:'afterGet', presetId:'builtin-mobile-bar', priority:40, active:true },
    { id:'rule-private-event', name:'Private Event', category:'privateEvent', minLeadDays:null, maxLeadDays:null, minContractValue:null, maxContractValue:null, contractValueBasis:'afterGet', presetId:'builtin-private-event', priority:50, active:true },
  ];
  return {
...
export async function saveQuickBooksDepositSettings(context: Context, settings: Partial<QuickBooksDepositSettings>) {
  ...
  const next: QuickBooksDepositSettings = {
    ...
    customPresets: cleanCustomPresets(settings.customPresets ?? current.customPresets),
    autoRules: cleanAutoRules(settings.autoRules ?? current.autoRules),
