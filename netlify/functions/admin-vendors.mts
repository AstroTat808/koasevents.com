...
function sanitize(body:any,current:any={}){
  ...
  return {
    ...
    insurance:{
      status:insuranceValues.has(body?.insurance?.status)?body.insurance.status:'not_requested',
      ...
      certificateUrl:clean(body?.insurance?.certificateUrl,1000),
      verifiedAt:clean(body?.insurance?.verifiedAt,60),
    },
    internal:{
