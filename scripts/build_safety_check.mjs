...
function checkEmailFeatureContracts() {
  const contracts = [
    ...
    ['netlify/functions/_shared/credential-health.ts', [
      ...
      ['safe repair audit storage', "'credential-health/safe-repair-history'"],
      ['safe repair audit writer', 'export async function recordCredentialSafeRepairAudit'],
      ['ranged credential reliability', 'export async function credentialReliabilityForRange'],
      ['Workspace Alert sync', 'syncCredentialWorkspaceAlerts'],
      ['credential reliability policy', 'export async function saveCredentialReliabilityPolicy'],
      ['credential reliability defaults', 'yellowBelow: 99'],
      ['credential reliability critical default', 'redBelow: 95'],
      ['credential reliability event storage', "'credential-health/reliability-events'"],
      ['combined credential incident timeline', 'function buildCredentialIncidentTimeline'],
    ]],
    ['netlify/functions/_shared/workspace-alert-lifecycle.ts', [
      ...
      ['credential alert timeline reader', 'export async function readCredentialWorkspaceAlertTimeline'],
    ]],
    ['netlify/functions/admin-health.mts', [
      ...
      ['credential threshold save action', "body?.action==='save-credential-reliability-thresholds'"],
      ['safe repair before snapshot', 'failedCredentials:beforeRows.filter'],
      ['safe repair retest detail', 'retested=attemptedIds.map'],
      ['safe repair manual remainder', 'stillRequiresMe:['],
      ['safe repair audit recording', 'recordCredentialSafeRepairAudit'],
      ['weekly executive summary response', 'weeklyExecutiveSummary'],
    ]],
    ['netlify/functions/_shared/weekly-health-summary.ts', [
      ['weekly System Health summary', 'export async function weeklySystemHealthExecutiveSummary'],
      ['weekly uptime comparison', 'function uptimeForRange'],
      ['weekly incident comparison', 'function incidentsForRange'],
      ['weekly repair comparison', 'function repairsForRange'],
      ['weekly unresolved problems', 'function unresolvedProblems'],
    ]],
    ...
    ['src/pages/admin/health/index.astro', [
      ...
      ['safe repair before-after report', 'data-fix-all-safe-report'],
      ['credential reliability view', 'data-credential-reliability-grid'],
      ['credential threshold save', 'data-save-credential-thresholds'],
      ['credential yellow threshold controls', 'dataset.credentialThresholdYellow'],
      ['credential red threshold controls', 'dataset.credentialThresholdRed'],
      ['credential incident timeline', 'data-credential-incident-timeline'],
      ['credential timeline provider filters', 'data-credential-timeline-provider'],
      ['credential timeline source filters', 'data-credential-timeline-source'],
      ['credential timeline filter count', 'data-credential-timeline-count'],
      ['weekly executive summary', 'data-weekly-executive-summary'],
      ['weekly uptime summary', 'data-weekly-uptime'],
      ['weekly credential summary', 'data-weekly-credential-reliability'],
      ['weekly incident summary', 'data-weekly-incidents'],
      ['weekly repair summary', 'data-weekly-repairs'],
      ['weekly unresolved summary', 'data-weekly-unresolved'],
      ['weekly comparison renderer', 'function renderWeeklyExecutiveSummary'],
